...
class BookingControllerIT {
    ...
    void createBooking_andGetByDriver_andGetStatus() throws Exception {
        BookingRequestDTO dto = new BookingRequestDTO(driver.getId(), charger.getId(), LocalDateTime.now(), LocalDateTime.now().plusHours(1));
        ...
        mockMvc.perform(get("/bookings/driver/" + driver.getId()))
                .andExpect(status().isOk())
				.andExpect(jsonPath("$[0].id").value(bookingId));

        // Get status
		String resp = mockMvc.perform(get("/bookings/" + bookingId + "/status"))
			.andExpect(status().isOk())
    ...
    void startStopCancelBooking() throws Exception {
        ...
        // Start charging
        mockMvc.perform(patch("/bookings/" + booking.getId() + "/start"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("CHARGING"));

        // Stop charging
        mockMvc.perform(patch("/bookings/" + booking.getId() + "/stop"))
                .andExpect(status().isOk())
