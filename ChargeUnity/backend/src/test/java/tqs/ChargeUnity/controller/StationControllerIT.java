...
class StationControllerIT {
    ...
    void createAndGetStation() throws Exception {
        ...
        String response = mockMvc.perform(post("/api/v1/station")
        ...
        // Get by ID
        mockMvc.perform(get("/api/v1/station/" + stationId))
    ...
    void updateAndDeleteStation() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/station/" + station.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(json))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("New Name"));

        mockMvc.perform(delete("/api/v1/station/" + station.getId()))
                .andExpect(status().isOk())
    ...
    void getStationsByLocation() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/station/41.15/-8.61/5"))
                .andExpect(status().isOk())
