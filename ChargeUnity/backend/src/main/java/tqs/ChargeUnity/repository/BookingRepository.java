...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
  ...
  boolean existsByChargerIdAndStartTimeLessThanEqualAndEndTimeGreaterThanEqual(
      Integer chargerId, LocalDateTime endTime, LocalDateTime startTime);

  // Optional: Retrieve all bookings for a charger between two dates (useful for operator
  // dashboards)
  List<Booking> findByChargerIdAndStartTimeBetween(
      Integer chargerId, LocalDateTime start, LocalDateTime end);

  @Query(
      "SELECT b FROM Booking b WHERE b.charger.id = :chargerId AND b.startTime < :end AND b.endTime"
          + " > :start")
